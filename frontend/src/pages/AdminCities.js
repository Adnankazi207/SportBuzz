...
const AdminCities = () => {
  ...
  const handleAddOrEdit = async () => {
    ...
    try {
      if (editingId) {
        await axios.put(
         `https://sportbuzz.onrender.com/api/admin/cities/${editingId}`,
          { name: cityName },
          { headers: { Authorization: `Bearer ${token} `} }
        );
        ...
        await axios.post(
          'https://sportbuzz.onrender.com/api/admin/cities',
          { name: cityName },
          { headers: { Authorization: `Bearer ${token} `} }
        );
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`https://sportbuzz.onrender.com/api/admin/cities/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
